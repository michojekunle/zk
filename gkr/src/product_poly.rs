...
impl<F: PrimeField> ProductPoly<F> {
    ...
    pub(crate) fn evaluate(&mut self, values: Vec<Vec<F>>) -> Vec<F> {
...
mod tests {
    ...
    fn test_evaluate() {
        ...
        let evals = product_poly.evaluate(values);
    ...
    fn test_multiple_polynomials() {
        let polys = (0..3).map(|i| {
            MultilinearPoly::new(vec![Fr::from(i as u64), Fr::from(i as u64 + 1)], 1)
        }).collect();
    ...
    fn test_empty_evaluation() {
        ...
        let evals = product_poly.evaluate(vec![]);
